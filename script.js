...
window.onload = () => {
  ...
  <p>🏠 Single-family: ${f.properties.sf_pct}%&nbsp;&nbsp;🏢 Multi-family: ${100 - f.properties.sf_pct}%</p>
        ...
        map.on('click', 'neighborhood-fill', (e) => {
          ...
          new mapboxgl.Popup()
            .setLngLat(e.lngLat)
            .setHTML(`<strong>${f.properties.name}</strong><br/><em>${f.properties.region}</em><br/>${f.properties.status}<br/>Poverty: ${f.properties.poverty_pct}%<br/>🏠 SF: ${f.properties.sf_pct}% / 🏢 MF: ${100 - f.properties.sf_pct}%`).addTo(map);
        });
      });
  });
};